...
>(({ children, ...props }, ref) => {
  ...
  const overflowingTriggers = useMemo(() => {
    return childArray.filter(
      (c, i) =>
        isValidElement(c) &&
        (c.type as any).displayName === "TabsTrigger" &&
        overflowIds.includes(i)
    ...
    return (
      <TabsPrimitive.Trigger
        ...
        className={cn(
          ...
          "inline-flex h-9 cursor-pointer items-center justify-center border-b-2 p-1 text-sm font-semibold text-tertiary underline-offset-4 disabled:cursor-not-allowed disabled:text-disabled data-active:border-b-2 data-active:border-neutral-dark  data-active:text-neutral-dark [&_svg]:size-4",
          "transition-colors group-data-[variant=filled]:data-active:bg-white",
          {
            "w-full justify-start self-start border-0 border-b-0 py-2 pl-0 font-medium text-primary hover:bg-config-sidebar-hover data-active:rounded data-active:border-none  data-active:bg-config-sidebar-active data-active:pl-0 data-active:font-semibold data-active:text-primary":
              variant === "nav",
          },
          {
            // filled variant
            "disabled:text-disabled group-[[data-variant]:not([data-variant=filled])]:mb-[-1px] data-[state=active]:z-10 group-data-[variant=filled]:rounded-t-lg group-data-[variant=filled]:border group-data-[variant=filled]:border-b-0 group-data-[variant=filled]:border-divider-light group-data-[variant=filled]:px-4 group-data-[variant=filled]:data-active:bg-white group-data-[variant=filled]:data-active:text-neutral-dark group-data-[variant=filled]:data-inactive:bg-body-main group-data-[variant=filled]:data-inactive:font-medium group-data-[variant=filled]:data-inactive:text-tertiary":
              true,
          },
          className
        )}
