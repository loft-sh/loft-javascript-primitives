...
function DataTableInner<TData>(
  ...
  const [openActionMenuRow, setOpenActionMenuRow] = useState<string | undefined>(undefined)
  ...
  return (
    <div className={cn("rounded-md border", className)}>
      ...
      <Table
        ...
        hasPagination={showPagination && table.getFilteredRowModel().rows.length > initialPageSize}
        ref={tableRef}
        style={{
          ...
          <TableBody>
            {shouldShowSkeleton ? (
              ...
              table.getRowModel().rows.map((row) => {
                ...
                const isActionMenuOpen = openActionMenuRow === row.id
                const shouldShowRowActions = isRowHovered || isActionMenuOpen
                const hasExpansionComponent = !!(expandedRows[key] && ExpansionComponent)

                const content = (
                  <DataTableRowContext.Provider
                    value={{
                      isHovered: shouldShowRowActions,
                      isActionMenuOpen,
                      setActionMenuOpen: (open) => {
                        setOpenActionMenuRow((currentOpenRow) => {
                          if (open) {
                            return row.id
                          }

                          return currentOpenRow === row.id ? undefined : currentOpenRow
                        })
                      },
                    }}>
                    <DataTableRow
                      ...
                      {row.getVisibleCells().map((cell) => {
                        ...
                        const hideActions =
                          isPinned && !shouldShowRowActions && !shouldMenuAppearOnHover
                          ...
                          <TableCell
                            ...
                            className={cn(
...

                              {
                                ...
                                "bg-table-cell-hover": shouldShowRowActions,
      ...
      {showPagination && table.getFilteredRowModel().rows.length > initialPageSize && (
        <TablePagination
          table={table}
