...
function DataTable<TData, TValue>({
  ...
  useEffect(() => {
    const determineIfWeShouldResetPageIndex =
      columnFilters.length > 0 && filteredDataLength < (data?.length || 0)

    if (determineIfWeShouldResetPageIndex) {
      table.setPageIndex(0)
    }
  }, [columnFilters.length, data?.length, filteredDataLength, table])
