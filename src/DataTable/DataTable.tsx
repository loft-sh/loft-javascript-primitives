...
import { DownloadOutlined, RightOutlined } from "@loft-enterprise/icons"
import {
  Button,
  cn,
...
function DataTable<TData, TSubRowData>({
      ...
      cell: ({ row }) => {
        ...
        return (
          <div
            ...
            <RightOutlined
              className={cn("size-3 select-none transition-transform *:size-3", {
                "rotate-90": expandedRows[rowKey],
              })}
            />
          </div>
        )
  ...
  const shouldShowSkeleton = loading && (data?.length === 0 || data === undefined)
