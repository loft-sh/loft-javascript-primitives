...
const NameCellMain: React.FC<NameCellMainProps> = ({ children }) => {
  return (
    <div className="relative flex w-full min-w-0 translate-y-[calc(var(--id-height)/2+1px)] flex-row items-center gap-2 text-sm transition-all group-hover:translate-y-0">
      {children}
    </div>
  )
}
