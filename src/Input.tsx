    ...
    return (
      <div
        ...
        className={cn(
          "group flex items-center justify-between gap-1 rounded-md border border-divider-main bg-white text-sm text-tertiary transition-colors focus-within:border-primary-main focus-within:outline-none hover:border-primary-light has-[:disabled]:cursor-not-allowed has-[:disabled]:bg-disabled-light [&_svg]:size-4 [&_svg]:fill-neutral-dark",
          {
            ...
            "border-none bg-transparent bg-none focus-within:border-none hover:outline hover:outline-[1px] hover:outline-primary-main":
              variant === "in-place",
            "border-danger-main focus-within:border-danger-main hover:border-danger-light focus:border-danger-main":
              error,
          },
          className
        )}>
        {preffix && (
          <div
            className={cn(
              "ml-3 flex select-none items-center",
              { "pointer-events-none": !prefixHint },
              prefixClassName
            )}>
            {prefixHint ? <Tooltip content={prefixHint}>{preffix}</Tooltip> : preffix}
          </div>
        )}
        <input
          ref={ref}
        ...
        {(suffix || (type === "password" && showToggle) || error) && (
          <div className={cn("mr-3 flex select-none items-center gap-2")}>
            {suffix}
            {statusText ? (
            ...
            {type === "password" && showToggle && (
              <button
                ...
                className="pointer-events-auto z-10 cursor-pointer rounded leading-[0] hover:opacity-80 focus-visible:opacity-80 focus-visible:outline focus-visible:outline-1 focus-visible:outline-primary-main">
                {showPassword ? (
