...
type BaseModalProps = Pick<DialogContentProps, "onOpenAutoFocus"> & {
  // Styles the outer modal box.
  className?: string

  // Styles the wrapper for the body.
  ...
  return (
    <DialogPrimitive.DialogOverlay
      ...
      className={cn(
        ...
        "h-[100vh] w-[100vw] animate-in-linear bg-gray-100/60"
      )}>
      {children}
    </DialogPrimitive.DialogOverlay>
  )
...
function InnerModalBox(
  {
    ...
    onOpenAutoFocus,
    style,
  }: { children?: React.ReactNode; className?: string; style?: CSSProperties } & Pick<
    DialogContentProps,
    "onOpenAutoFocus"
  >,
  ref: ForwardedRef<HTMLDivElement>
  ...
  return (
    <DialogPrimitive.Content
      ...
      className={cn(
        "flex max-h-[calc(100vh-2rem)] flex-col rounded-lg bg-white shadow-small outline-none",
        className
      )}>
      {children}
    </DialogPrimitive.Content>
...
function ModalBody({ children, className }: { children?: React.ReactNode; className?: string }) {
  return (
    <div
      className={cn(
        "flex min-h-0 w-full flex-col overflow-y-auto px-6 py-2 text-sm font-normal text-primary",
        className
      )}>
      {children}
    </div>
    ...
    <>
      {leftAction && (
        <Button
          ...
          {leftAction.Icon && <leftAction.Icon aria-hidden="true" />} {leftAction.label}
        </Button>
      )}
      <div className={"ml-auto flex flex-row items-center gap-2"}>
        {secondaryAction && (
          <Button
            {...(secondaryAction.style ?? { variant: "ghost" })}
            disabled={secondaryAction.disabled}
            onClickAsync={secondaryAction.onClick}>
            {secondaryAction.Icon && <secondaryAction.Icon aria-hidden="true" />}{" "}
            {secondaryAction.label ?? "Cancel"}
          </Button>
        )}
        {primaryAction && (
          <Button
            {...(primaryAction.style ?? { appearance: "brand" })}
            disabled={primaryAction.disabled}
            onClickAsync={primaryAction.onClick}>
            {primaryAction.Icon && <primaryAction.Icon aria-hidden="true" />} {primaryAction.label}
          </Button>
        )}
      </div>
    </>
    ...
    <DialogPrimitive.Root modal={true} open={visible} onOpenChange={onOpenChange}>
      <DialogPrimitive.Portal>
        <ModalMask>
          <ModalBox
            onOpenAutoFocus={onOpenAutoFocus}
            className={cn("min-w-[30rem] max-w-[33rem]", className)}
            style={style}>
            {(title || warning) && (
              <ModalTitle>
                {warning && (
                  <div
                    className={
                      "text-[height:inherit] mr-2 flex h-[1lh] flex-col items-center justify-center leading-[inherit]"
                    }>
                    <WarningOutlined className={"w-5"} />
                  </div>
                )}
                {title}
              </ModalTitle>
            )}
            <ModalBody className={cn(!title && !warning && "pt-6", bodyClassName)}>
              {children}
            </ModalBody>
            {(customActions || primaryAction || secondaryAction || leftAction) && (
              <ModalActions>
                {customActions ?? (
                  <ModalDefaultActions
