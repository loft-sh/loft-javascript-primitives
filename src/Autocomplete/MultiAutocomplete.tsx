...
const MultiAutocompleteItem = memo(
  forwardRef<HTMLDivElement, MultiAutocompleteItemProps>(function InnerMultiAutocompleteItem(
    { showTooltips, value, onRemoveRequested, className, preventRemove },
    ref
  ) {
    ...
    return (
      <div
        ...
        <Tooltip wrappingTriggerDiv={false} content={renderTooltip ? value : undefined}>
          <span
            ref={spanRef}
...
function MultiAutocompleteInput({
  ...
  const { inputValue, setInputValue, focusFirstSuggestion, focusLastSuggestion } =
    useContext(SuggestionContext)
  const { id } = useContext(AutocompleteContext)
  ...
  // Memoize the display items so we don't need to render all of them while we type.
  // Since the component itself is memoized, main reason for this is the callback.
...
function MultiAutocompleteTrigger({
  ...
  const displayItems = useMemo(() => {
    return visibility.visibleItems.map((v, i) => (
      <MultiAutocompleteItem
        value={v}
        key={v}
        onRemoveRequested={() => {
  ...
  }, [values, visibility.visibleItems, onValuesChanged])
...
export function MultiAutocomplete({
  ...
  const suggestionContextRef = useRef<SuggestionContextValue | null>(null)

  const [open, setOpen] = useState<boolean>(false)
  const id = useId()

  const onCloseRequested = useCallback(() => {
    // When the autocomplete gets closed and we still have some input value, we add it as a new item.
    if (suggestionContextRef.current && suggestionContextRef.current.inputValue) {
      const trimmed = suggestionContextRef.current.inputValue.trim()
  ...
  return (
    <AutocompleteContextProvider
      ...
      id={id}>
      {open ? (
        <SuggestionContextProvider ref={suggestionContextRef}>
          <AutocompleteAnchor>
