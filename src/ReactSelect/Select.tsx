...
import { CommonSelectEmptyState, SelectEmptyStateProps } from "../Select"
    ...
    <div
      ...
      {memoizedErrorContent && (
        <div
          ...
          <Tooltip className="z-40" content={memoizedErrorContent}>
            <CloseCircleFilled />
          </Tooltip>
        </div>
      )}
...
const SelectSingleValue = <OptionType extends TSelectOptionType, IsMulti extends boolean>({
  ...
  return (
    <div
      className={cx(
        "pointer-events-none z-40 flex select-none items-center text-sm data-[selected=true]:bg-primary-main",
        {
          "absolute w-[85%] justify-between": !isPlain,
...
const SelectMultiValue = <IsMulti extends boolean>({
  ...
  return (
    <Chip
      size="small"
      className="h-[22px] truncate border-neutral-light bg-neutral-main text-white [&_svg]:size-3">
...
>(props: TSelectMultiValueContainerProps<OptionType, IsMulti, Group>): React.ReactElement {
  return <div className="z-10 flex flex-wrap items-center gap-1">{props.children}</div>
}
...
>(
  ...
  return (
    <div
      ...
      className={cn(
        props.className,
        `border-gray-300 select-menu-list z-40 mt-1 overflow-y-auto rounded-md border bg-white shadow-lg`,
        {
          "cursor-auto": !props.options.length,
        }
      )}>
      {!props.options.length && <CommonSelectEmptyState {...props.emptyStateProps} />}
      {props.children}
    </div>
  )
}
...
const SelectOptionBase = <OptionType extends TSelectOptionType, IsMulti extends boolean = false>(
  ...
  return isDisabled ? (
    <Tooltip className="z-tooltip max-w-[40ch]" content={disabledMessage}>
      {content}
    </Tooltip>
  ) : (
    content
...
const SelectControl = <OptionType extends TSelectOptionType, IsMulti extends boolean>(
  ...
  return (
    <Tooltip className="z-40" content={isDisabledTooltip}>
      <div
...
/**
 * @deprecated Use the radix Select / new MultiSelect instead.
...
const Select = React.memo(
  <OptionType extends TSelectOptionType, IsMulti extends boolean>({
    ...
    const memoizedContainer = useMemo(
      () =>
        (props: TSelectContainerProps<OptionType, IsMulti> & { classNames?: ClassNamesConfig }) => (
          <SelectContainer
            {...props}
            variant={variant}
            error={error}
    ...
    return (
      // TS Compiler is triggering an error here.
      // @ts-ignore
      <>
        <div className="w-full" ref={selectRef}>
          <SelectComponent
            {...selectProps}
            menuPortalTarget={portalRef.current ?? undefined}
            classNames={{
              menu: () => "z-40",
              ...props.classNames,
            }}
        ...
        {usePortal &&
          createPortal(
            <div className={"fixed left-0 top-0 z-40"} ref={portalRef}></div>,
