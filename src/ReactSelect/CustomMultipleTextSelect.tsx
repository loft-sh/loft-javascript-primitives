...
import { CommonSelectEmptyState } from "../Select/Common/CommonSelectEmptyState"
...
/**
 * @deprecated Use the new MultiSelect instead.
 */
const MultipleTextSelect = ({
  ...
  const memoizedMenuList = useMemo(
    ...
    [searchable, allowClearSelection, requireSelection, showSelectAllCheckbox, menuListClassName]
  )
...
const ValueContainer = ({
  ...
  return (
    <div
      onClick={onMenuOpen}
      className={cn(
        "value-container flex w-full select-none items-center gap-1.5 overflow-hidden pl-3 text-sm text-tertiary",
        { "text-primary": options.length }
      )}>
      {inputPrefix}
      <span className="value-container-label block max-w-full flex-grow overflow-hidden overflow-ellipsis whitespace-nowrap">
        {label}
      </span>
      <span className="flex-shrink-0 py-4">{children}</span>
    </div>
  )
}
...
const SelectOption = <OptionType extends MultipleSelectOptionType>(
  ...
  return (
    <div
      ...
      <div
        ref={innerRef}
        className={cx(`flex flex-grow flex-row items-center justify-start gap-1 px-3 py-2`)}>
        <span className="text-tertiary">{props.data.extraArgs?.icon}</span>
        {children}
      </div>
    </div>
  )
...
function MenuList<OptionType extends MultipleSelectOptionType>(
  ...
  return (
    <div
      ...
      {...props.innerProps}
      className={cn(
        "border-gray-300 rounded-md border bg-white shadow-lg focus:outline-none",
        {
      ...
      {!filteredOptions.length && <CommonSelectEmptyState />}
...
const SelectAllCheckbox = ({
  ...
  const handleChange = useCallback(() => {
    ...
    if (allowClearSelection && selectedCount === options.length) {
      setValue([] as MultiValue<OptionType>, "select-option", undefined)
    }
