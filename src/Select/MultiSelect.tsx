...
import { CloseCircleOutlined, CloseOutlined } from "@loft-enterprise/icons"
  ...
  return (
    <div
      ...
      {chip ? (
        ...
        <div className={cn("inline-flex flex-row items-center truncate")} title={titleText}>
          {label}
          {hasNext ? "," : ""}
        </div>
      )}
    ...
    return (
      <PopoverContent
        className={cn(
          ...
          "w-[var(--radix-popover-trigger-width)] overflow-y-auto p-0 [scrollbar-width:thin]",
          className
        )}
        ref={ref}
        align="start"
...
export const MultiSelectItem = forwardRef<HTMLDivElement, MultiSelectItemProps>(
  ({ value, className, children, tooltip }, ref) => {
    const { setLabelLookup, toggleOption, values } = useContext(MultiSelectContext) ?? {}
